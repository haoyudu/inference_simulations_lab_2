...
```{r}
tea_data_highn <- make_tea_data(n_total = 48, delta = .5)
```
...
```{r}
mean_by_condition <- tea_data %>% 
  group_by(condition) %>% 
  summarize(mean = mean(rating), sd = sd(rating), num_participants = length(rating))

mean_by_condition_highn <- tea_data_highn %>% 
  group_by(condition) %>% 
  summarize(mean = mean(rating), sd = sd(rating), num_participants = length(rating))
```
...
An independent samples t-test was conducted to compare tea ratings between the milk-first condition (M = `r mean_by_condition$mean[1]`, SD = `r mean_by_condition$sd[1]`) and tea-first condition (M = `r mean_by_condition$mean[2]`, SD = `r mean_by_condition$sd[2]`). The difference in ratings was not statistically significant, t(`r out_low_n$parameter`) = `r out_low_n$statistic`, p = `r out_low_n$p.value`, 95% CI \[`r out_low_n$conf.int[1]`, `r out_low_n$conf.int[2]`\]. An independent samples t-test was conducted to compare tea ratings between the milk-first condition (M = `r mean_by_condition_highn$mean[1]`, SD = `r mean_by_condition_highn$sd[1]`) and tea-first condition (M = `r mean_by_condition_highn$mean[2]`, SD = `r mean_by_condition_highn$sd[2]`). The difference in ratings approached but did not reach statistical significance, t(`r out_high_n$parameter`) = `r out_high_n$statistic`, p = `r out_high_n$p.value`, 95% CI \[`r out_high_n$conf.int[1]`, `r out_high_n$conf.int[2]`\].

### Simulate 1000 experiments

Now let's simulate 1000 experiments...where you have 18 participants per experiment with an average difference of 1.5 points in tea deliciousness.
...
```{r}
tea_data_summary <- samps |> # start with dataframe 1000 experiments 18 people each 
  group_by(sim, condition) |> # intuitively, grouping first by experiment then by condition in each experiment (2000 groups)
  summarise(mean_rating = mean(rating)) |> # calculate mean of the ratings in each condition group, 2 rows per simulation
  group_by(sim) |> # one group per simulation
  summarise(delta = mean_rating[condition == "milk first"] -
              mean_rating[condition == "tea first"])   # calculate the difference between conditions for each experiment, delta is the effect size, resulting in one row per simulation
```
...
```{r}
# Large effect, low noise
samps_large <- tibble(sim = 1:1000) |> 
  mutate(data = map(sim, \(i) make_tea_data(n_total = 48, delta = 3, sigma = 0.5)))

# Small effect, high noise
samps_small <- tibble(sim = 1:1000) |> 
  mutate(data = map(sim, \(i) make_tea_data(n_total = 18, delta = 0.5, sigma = 2)))
...
```{r}
tea_data_summary_null |> 
  ggplot(aes(x = delta)) +
  geom_histogram(bins = 20, alpha = 0.7) +
  labs(title = "Null Distribution")


summary(tea_data_summary_null$delta)
```
...
To do: check you understanding -- what is "sim" here? each experiment

```{r}
tea_data_highn_shuffled_summary <- tea_data_highn_shuffled %>%
  group_by(condition_shuffled, sim) %>% 
  summarize(mean_rating = mean(rating),
            sd = sd(rating)) # for each exp and each shuffled condition, calculate the mean rating, 2 per simulation
...
To do: what does this histogram look like and why? Even though individual ratings might not be normally distributed, the distribution of differences between group means tends to be normal.
...
Honestly, I doubt you will get this far in lab (HA! we got here) -- but if you do, there are two extra pieces of material that are I think are foundation material.
...
### Plotting CIs by hand for each condition
...
each dot is one individual person's rating, in the first CI graph, the range 95% confidence interval for the mean (we are 95% confident that true population mean lies within this range), and in the SE graph, the range is +- 1 standard error, which is the typical variability in the sample mean if we repeated the experiment many times. CI is about true population mean, and SE is about sampling variability.
...
ggplot(data = tea_data, aes(x=condition, y=rating, col=condition))  +
  ...
  ylim(0,10) +
  ggtitle('Tea ratings low N with CI')
```
...
ggplot(data = tea_data_highn, aes(x=condition, y=rating, col=condition))  +
  ...
  ylim(0,10) +
  ggtitle('Tea ratings with 95% CI')
```
...
all_results %>%
  ...
  labs(title = "Distribution of P-values Under Null Hypothesis",
       x = "P-value",
       y = "Frequency") +
  theme_minimal()
```
